...
export default () => {
  ...
  const onDeleteFile = id => () => {
    dispatch(GalleryAction.deleteMultimedia(id));
  };
  useEffect(() => {
      ...
      <Col>
                  ...
                  {galleryMultimedia.map((src, index) => (
                    <Col xs={6} className="p-1 position-relative" key={index}>
                      <FontAwesomeIcon
                        ...
                        onClick={onDeleteFile(galleryMultimedia[index].id)}
                      />
                      <Card
        ...
        <Row className="mt-2">
          <Col className="d-flex justify-content-center">
            <Button onClick={() => history.push('/gallery/add')} color="info" size="lg" block style={{ maxWidth: 650 }}>
              Añadir Multimedia
            </Button>
          </Col>
        </Row>
